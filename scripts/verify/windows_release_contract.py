...
from qwenpaw.app.go_claw_updates import verify_minisign  # noqa: E402
...
def _validate_credentials(data: bytes) -> None:
    try:
        ...
        raise ValueError("Full ZIP credentials are structurally invalid") from exc
    if (
        ...
        raise ValueError("Full ZIP credentials do not match the delivery contract")
...
def _verify_full_zip(full_zip: Path, pubkey: str) -> dict[str, object]:
    ...
    if missing:
        raise ValueError(f"Full ZIP is missing required files: {sorted(missing)}")
    for relative in relative_data:
        ...
        if (
            ...
            raise ValueError(f"Full ZIP contains forbidden material: {relative}")
    ...
    _validate_credentials(relative_data["Portable/GO-CLAW-Config/credentials.json"])
    ...
    if manifest.get("webView2", {}).get("sha256") != _sha256(relative_data[webview_path]):
    ...
    if paths_in_order != sorted(paths_in_order, key=lambda value: value.encode()):
    ...
    expected_manifest_paths = set(relative_data) - {"MANIFEST.json", "SHA256SUMS.txt"}
    actual_manifest_paths = {entry.get("path") for entry in manifest_entries if isinstance(entry, dict)}
    ...
    return {"version": str(manifest.get("version")), "files": len(relative_data)}
...
def _verify_update_payload(payload: Path) -> None:
    ...
    for path in payload.rglob("*"):
        ...
        if parts & FORBIDDEN_UPDATE_PARTS:
            raise ValueError(f"update payload contains forbidden path: {path.name}")
...
def verify_release_contract(
    *,
    ...
    resolved = {label: _require_file(path, label) for label, path in paths.items()}
    pubkey = _read_pubkey(resolved["public key config"])
    installer_sig = resolved["installer signature"].read_text(encoding="utf-8-sig").strip()
    update_sig = resolved["update signature"].read_text(encoding="utf-8-sig").strip()
    try:
        verify_minisign(resolved["installer"].read_bytes(), installer_sig, pubkey)
        verify_minisign(resolved["update"].read_bytes(), update_sig, pubkey)
    except ValueError as exc:
        raise ValueError("Windows release signature verification failed") from exc

    try:
        latest = json.loads(resolved["latest.json"].read_text(encoding="utf-8"))
    ...
    if platform.get("signature") != update_sig:
        raise ValueError("latest.json signature does not equal the update signature")
    ...
    if "Microsoft Corporation" not in webview2_authenticode_subject:
        raise ValueError("WebView2 Authenticode subject is not Microsoft Corporation")
