...
2026-09-05 现场补充：本次带更新器 2.0.1 同基准在 NTFS 和 F exFAT 均完成历史 staging 包安装及
自动重启，但媒体插件禁用/模型档位加载中，产品 readiness 仍不通过；非空聊天历史未覆盖。
本文以下生产 2.1.0/软链陈述是历史记录，当前公共 manifest 的只读复核见
`GO-CLAW-项目事实与发布基线.zh.md` §1，不能按历史记录修改生产。完整证据见
`incidents/2026-09-04-v201-clean-sample-preflight.zh.md`；§7 取证顺序继续有效，事故未关闭。
