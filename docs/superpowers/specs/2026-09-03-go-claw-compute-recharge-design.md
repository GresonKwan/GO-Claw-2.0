...
已确认：单笔最低 ￥1、最高 ￥100,000；快捷金额 ￥10/￥50/￥100/￥200；用户展示“￥1对应500万算力”；微信支付商户已开通。

其余参数不阻塞代码骨架，但上线前必须签字确认：

1. 每账户每日累计风控上限；
2. 微信商户 `mchid`、绑定 `appid`、证书/平台公钥方案、APIv3 密钥和正式 notify 域名；
3. 发票开具主体、入口与时限；
4. 未消费额度、部分消费额度与异常订单的退款政策；
5. 账户/U 盘遗失后的身份核验和 token 吊销流程；
6. 客服联系方式、服务时间和账务争议升级人；
7. WORM 保存年限、数据库备份保存年限和 PII 保存年限；
8. 是否接受首期“NewAPI 结果不确定进入人工复核”，或必须先建设下游幂等适配层再上线。
