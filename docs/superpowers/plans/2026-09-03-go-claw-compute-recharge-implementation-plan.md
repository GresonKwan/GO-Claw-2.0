...
- [x] 项目负责人确认微信支付商户已经开通；
- [x] 核验经营主体与商户号 `1749383281`，并锁定“未来AI工坊” `wx04d715aaaa2bd0ed` 为 GO CLAW 唯一下单 AppID；
- [x] “未来AI工坊”小程序管理员已完成跨主体关联授权；
- [x] Native 支付已开通；商户 API 证书、微信支付公钥与公钥ID已核验，APIv3 密钥已由负责人安全保存；
- [ ] 在 `goclaw.host` 部署并验证正式 HTTPS notify 路由，保持充值开关关闭直至完整闭环验收；
- [ ] 创建独立测试 merchant/config 或受控小额测试流程；
- [x] 财务规则确认：每日累计充值上限 ￥100,000，退款转客服处理，暂不提供发票；
