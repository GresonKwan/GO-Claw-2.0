...
export function AgentModal({
  ...
  const selectedTier = Form.useWatch("model_tier", form) ?? "economy";
  useEffect(() => {
    if (!open) return;
  ...
  return (
    ...
    >
      <Form form={form} layout="vertical" autoComplete="off">
        ...
        <Form.Item name="model_tier" hidden initialValue="economy">
          <Input />
        </Form.Item>
        <Form.Item label={t("agent.model")}>
          <div className={styles.modelTierPicker}>
            {MODEL_TIER_PRESENTATION.map((tier) => (
              <button
                key={tier.id}
                type="button"
                className={`${styles.modelTierCard} ${
                  selectedTier === tier.id ? styles.modelTierCardSelected : ""
                }`}
                onClick={() => form.setFieldValue("model_tier", tier.id)}
              >
                <img src={MODEL_TIER_ICONS[tier.icon]} alt={tier.label} />
                <span>
                  <strong>{tier.label}</strong>
                  <small>{tier.description}</small>
                  {tier.warning && <small>{tier.warning}</small>}
                </span>
              </button>
            ))}
          </div>
