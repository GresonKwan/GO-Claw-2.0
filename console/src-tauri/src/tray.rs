...
pub(crate) fn show_main_window(app: &tauri::AppHandle) {
    ...
    } else if app.state::<PortableRuntime>().state().is_some() {
        if let Some(port) = app.state::<backend::BackendState>().port() {
            if let Err(error) = crate::client::open_browser(app, port) {
                log::warn!("[portable-client] failed to reopen browser from tray: {error}");
            }
        }
    }
}
