...
pub(super) fn watch(
    ...
    tauri::async_runtime::spawn(async move {
        ...
        let mut startup_error_shown = false;
        ...
        while let Some(event) = rx.recv().await {
            match event {
                ...
                CommandEvent::Error(message) => {
                    ...
                    let message = format!("backend process error: {message}");
                    app.state::<BackendState>()
                        .set_error_if_current(generation, message.clone());
                    if should_show_startup_failure(
                        app.state::<BackendState>().is_current(generation),
                        became_ready,
                        startup_error_shown,
                    ) {
                ...
                CommandEvent::Terminated(payload) => {
                    let dialog_message = termination_summary(&payload);
                    ...
                    if stopping {
                        ...
                        state.set_error_if_current(generation, message.clone());
                        if should_show_startup_failure(
...
fn should_show_startup_failure(
    is_current_generation: bool,
    became_ready: bool,
    already_shown: bool,
) -> bool {
    is_current_generation && !became_ready && !already_shown
}
...
fn termination_message(payload: TerminatedPayload, last_stderr: &str) -> String {
    let mut message = termination_summary(&payload);
...
mod tests {
    ...
    #[test]
    fn stale_generation_never_shows_startup_failure() {
        assert!(!should_show_startup_failure(false, false, false));
        assert!(!should_show_startup_failure(true, true, false));
        assert!(!should_show_startup_failure(true, false, true));
        assert!(should_show_startup_failure(true, false, false));
    }
}
