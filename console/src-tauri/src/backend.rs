...
use crate::portable::PortableRuntime;
...
impl BackendState {
    ...
    fn set_port_if_current(&self, generation: u64, port: u16) -> bool {
        self.with_inner(|inner| {
            if self.is_current(generation) && inner.port.is_none() {
...
pub(crate) fn setup(app: &mut tauri::App) -> Result<(), Box<dyn std::error::Error>> {
    let file_target = match app.state::<PortableRuntime>().state() {
    ...
    app.handle().plugin(
        tauri_plugin_log::Builder::default()
            .clear_targets()
            .targets([Target::new(TargetKind::Stdout), file_target])
...
mod tests {
    ...
    #[test]
    fn ready_port_is_accepted_once_per_current_generation() {
        let state = BackendState::default();
        let first_generation = state.next_generation();

        assert!(state.set_port_if_current(first_generation, 51001));
        assert!(!state.set_port_if_current(first_generation, 51002));
        assert_eq!(state.port(), Some(51001));

        let second_generation = state.next_generation();
        state.clear_startup_state();
        assert!(!state.set_port_if_current(first_generation, 51003));
        assert!(state.set_port_if_current(second_generation, 51004));
        assert_eq!(state.port(), Some(51004));
    }
}
