...
def test_signed_windows_build_stages_exact_public_assets():
    ...
    assert "QwenPaw-Tauri-${{ steps.version.outputs.version }}-Windows-setup.exe" not in workflow
...
def test_publish_requires_exact_credential_free_windows_assets():
    ...
    attach_step = workflow.split("Attach signed GO CLAW Windows assets", 1)[1].split(
        "upload-oss:", 1
    )[0]
