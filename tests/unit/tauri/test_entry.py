...
def test_main_prepares_portable_history_before_backend(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(entry, "_install_certifi_env", lambda: None)
    monkeypatch.setattr(
        "qwenpaw.portable.is_portable_mode",
        lambda: True,
    )
    monkeypatch.setattr(
        "qwenpaw.portable.prepare_portable_location_history",
        lambda path: calls.append(("portable", path)),
    )
    monkeypatch.setattr(
        "qwenpaw.utils.platform.auto_disable_sandbox_on_windows",
        lambda: calls.append(("sandbox", None)),
    )
    monkeypatch.setattr(
        entry,
        "_run_backend_server",
        lambda level: calls.append(("backend", level)),
    )
    monkeypatch.setattr("qwenpaw.constant.WORKING_DIR", tmp_path)
    monkeypatch.delenv("QWENPAW_LOG_LEVEL", raising=False)
