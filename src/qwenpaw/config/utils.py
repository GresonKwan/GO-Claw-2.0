...
def _rebase_portable_path(value: str) -> str:
    """Rebase a path below an old portable root onto the current root."""
    candidate = Path(value).expanduser()
    current = Path(WORKING_DIR).expanduser()
...
def _normalize_working_dir_bound_paths(data: object) -> object:
    """Normalize QwenPaw-owned paths to the current WORKING_DIR.
